...
##                          base model                          --
##----------------------------------------------------------------
runBaseModel <- function(train, test, features, response, time, cutoff_metric, exclude_algos,save.location) {
  automl <- data.frame()
  factor.models <- c("XGBoost", "DeepLearning")
  ## model
  model.fs <- h2o.automl(
    x = features,
    y = response,
