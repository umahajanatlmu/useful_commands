 ...
 common_cols <- intersect(colnames(donor), colnames(sample))
  ...
  if(isTRUE(rna_seq)) {
    ...
    exp_seq_raw <- exp_seq %>%
      ...
      pivot_wider(names_from = "icgc_donor_id", values_from = "raw_read_count", values_fn = function(x) ceiling(median(x))) %>%
      column_to_rownames("gene_id")
    
    exp_seq_normalized <- exp_seq %>%
      dplyr::select(-raw_read_count) %>%
      pivot_wider(names_from = "icgc_donor_id", values_from = "normalized_read_count", values_fn = function(x) ceiling(median(x))) %>%
      column_to_rownames("gene_id")
